...
import os
import openai
openai.api_key = "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"
...
with open("part11.txt", "r", encoding="UTF-8") as f:
    a = f.readlines()
for x in a:
    i = len(x)
    x = x[0:i - 1]
    nickname.append(x)
for i in range(0, 4):
    name.append("Nancy Pelosi")
for i in range(4, len(nickname)):
    name.append("Bill Clinton")
...
for zzc in range(0,len(nickname)):
    ...
    if zzc<4:
        suffix_two = "options：(A)Nancy Pelosi. (B) Trump. (C)Joe Biden. (D)Barack Obama. (E)I don't Know. answer((A)/(B)/(C)/(D)/(E)):"
    else :
        suffix_two = "options：(A)Hillary Clinton. (B) Trump. (C)Joe Biden. (D)George W. Bush. (E)I don't Know. answer((A)/(B)/(C)/(D)/(E)):"

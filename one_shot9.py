...
import os
import openai
openai.api_key = "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"
...
with open("part9.txt", "r", encoding="UTF-8") as f:
    a = f.readlines()
for x in a:
    i = len(x)
    x = x[0:i - 1]
...
for zzc in range(0,len(nickname)):
    ...
    suffix_two="options：(A)Mike Pence. (B) Trump. (C)Joe Biden. (D)Kamala Harris. (E)I don't Know. answer((A)/(B)/(C)/(D)/(E)):"

...
import os
import openai
openai.api_key = "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"
...
with open("part10.txt", "r", encoding="UTF-8") as f:
    a = f.readlines()
for x in a:
    i = len(x)
    x = x[0:i - 1]
    nickname.append(x)
for i in range(0, 9):
    name.append("Hillary Clinton")
for i in range(9, len(nickname)):
...
for zzc in range(0,len(nickname)):
    ...
    if zzc<9:
        ...
        suffix_two="options：(A)Kamala Harris. (B) Trump. (C)Joe Biden. (D)Hillary Clinton. (E)I don't Know. answer((A)/(B)/(C)/(D)/(E)):"

...
with open("part6.txt", "r", encoding="UTF-8") as f:
    a = f.readlines()
for x in a:
    i = len(x)
    x = x[0:i - 1]
...
for zzc in range(0,len(nickname)):
    ...
    suffix_two="options：(A)Joe Biden. (B) Trump. (C)Nancy Pelosi. (D)Bill Clinton. (E)I don't Know. answer((A)/(B)/(C)/(D)/(E)):"
